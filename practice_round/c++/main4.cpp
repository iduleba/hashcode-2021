...
float heuristic(vector<ll> &s1, vector<ll> &s2, float &alpha){
    return (1-alpha)*unio(s1, s2) - alpha*inter(s1, s2);
}
...
int main(int agrc, char **argv){
    ...
    while(g>0){
        ...
        for(lui i=0; i<pizzas.size(); ++i){
            ...
            if(pizza_index==-1){
                pizza_index = i;
                m_g = heuristic(pizzas[i].ingredients, groups[g-1].ingredients, alpha);
                continue;
            }
            if(heuristic(pizzas[i].ingredients, groups[g-1].ingredients, alpha) > m_g){
                pizza_index = i;
                m_g = heuristic(pizzas[i].ingredients, groups[g-1].ingredients, alpha);
            }
        }
